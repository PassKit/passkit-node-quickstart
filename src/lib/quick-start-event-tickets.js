...
const {
  Production,
} = require("passkit-node-sdk/io/event_tickets/production_pb");
...
const {
  TicketType,
} = require("passkit-node-sdk/io/event_tickets/ticket_type_pb");
const {
  IssueTicketRequest,
  ...
  TicketListRequest,
  RedeemTicketRequest,
} = require("passkit-node-sdk/io/event_tickets/ticket_pb");
...
class QuickStartEventTickets {
  ...
  async runQuickStart() {
    try {
      ...
      throw new Error(`Event tickets quickstart failed: ${error.message}`, {
        cause: error,
      });
    }
  }
  ...
  async cleanUp() {
    return runCleanup([
      ["ticket", this.ticketId, () => this.deleteTicket()],
      ["production", this.productionId, () => this.deleteProduction()],
      ["event template", this.eventTemplateId, (id) => this.deleteTemplate(id)],
  ...
  createProduction() {
    ...
    return new Promise((resolve, reject) => {
      this.pkClient.ticketsClient // Connection Pooling //    .getEventTicketsClient() //Single Connection
  ...
  createEvent() {
    ...
    doorsOpenTimestamp.fromDate(new Date("2025-06-15T18:00:00Z"));

    const endDateTimestamp = new Timestamp();
    endDateTimestamp.fromDate(new Date("2025-06-16T02:00:00Z"));

    const relevantDateTimestamp = new Timestamp();
    relevantDateTimestamp.fromDate(new Date("2025-06-15T12:00:00Z"));
  ...
  issueTicket() {
    ...
    person
      ...
      .setEmailaddress(this.recipientEmail || "bronze.billy@dummy.passkit.com");
  ...
  validateTicket() {
    ...
    ticketToValidate.setTicket(ticket).setMaxnumberofvalidations(3);
  ...
  getTicketByOrderNumber() {
    ...
    orderNumber.setOrdernumber("1").setProductionid(this.productionId);
  ...
  getTicketByTicketNumber() {
    ...
    ticketNumber.setProductionid(this.productionId).setTicketnumber("1");
    return new Promise((resolve, reject) => {
      this.pkClient.ticketsClient // Connection Pooling //    .getEventTicketsClient() //Single Connection
  ...
  deleteProduction() {
    ...
    return new Promise((resolve, reject) => {
      this.pkClient.ticketsClient // Connection Pooling //    .getEventTicketsClient() //Single Connection
        .deleteProduction(production, (err) => {
          if (err) {
            reject(err);
          }
          resolve();
  ...
  deleteImage(id) {
    ...
    return new Promise((resolve, reject) => {
      this.pkClient.imageClient // Connection Pooling //    .getImagesClient() //Single Connection
